...
output = gen_learnWeight(data=data_Y, exp='1', m_dists=[u_dist_cf + np.eye(data_Y.nM) * 2], n_dists=[np.ones((data_Y.nN, data_Y.nN)) + np.eye(data_Y.nN)], _cf=False, nRef=-1, nEpoch=50, lRate=0.01, batchSize=-1, title='All')
predictions, metrics = gen_model(**output)
...
predictions, metrics = gen_model(**output)
output = gen_learnWeight(data=data_Y, exp='1', m_dists=[u_dist_sat], n_dists=[dist_review], _cf=False, nRef=-1, nEpoch=50, lRate=0.01, batchSize=-1, title='All')
predictions, metrics = gen_model(**output)
